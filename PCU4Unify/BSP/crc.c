...
{
//MODBUS CRC-16 8005 逆序

  return GetCrc_16(pdata, len, 0xFFFF, g_McRctable_16);
}

unsigned short GetCrc_16_for_APP(int Addr,int nLength, 
																	unsigned short init, 
																	const unsigned short *ptable)
{
  unsigned short cRc_16 = init;
  unsigned char temp;  
	unsigned int num;   //长度2取整
	unsigned char leave; //长度2取余
	uint32_t ReadAddr;   //读FLASH地址
	unsigned short carry; //读半字
  ...
  while(num-- > 0)
  {
		carry = STMFLASH_ReadHalfWord(ReadAddr);
    temp = cRc_16 & 0xFF; 
    cRc_16 = (cRc_16 >> 8) ^ ptable[(temp ^ (carry &0xFF)) & 0xFF];
		
		temp = cRc_16 & 0xFF; 
    cRc_16 = (cRc_16 >> 8) ^ ptable[(temp ^ ((carry >>8) &0xFF)) & 0xFF];
		ReadAddr +=2;
  }
	if(leave !=0)
	{
...
//MODBUS CRC-16 8005 逆序
  ...
  //return GetCrc_16(pdata, len, 0xFFFF, g_McRctable_16);
	return GetCrc_16_for_APP(Addr,len, 0xFFFF, g_McRctable_16);
}
